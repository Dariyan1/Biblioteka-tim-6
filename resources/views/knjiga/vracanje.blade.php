            ...
            <div class="heading">
                <div class="flex flex-row justify-between border-b-[1px] border-[#e4dfdf]">
                    ...
                    <div class="pt-[24px] mr-[30px]">
                        ...
                        <a href="{{route('knjiga.rezervisi',$knjiga->id)}}" class="hover:text-blue-600 inline ml-[20px] pr-[10px]">
            ...
            <div class="scroll height-dashboard px-[30px]">
                ...
                <div
                    ...
                    <table class="min-w-full shadow-lg" id="vratiKnjiguTable">
                        ...
                        <form method="post" action="{{route('knjiga.vrati',$knjiga->id)}}" class="text-gray-700 forma">
                        @csrf 
                        @method('POST')
                        @foreach($izdate as $i)
                            ...
                            <div class="fail" id="validateNazivKnjiga">
                                 @error('vrati')@php echo'Morate selektovati knjigu koju vracate'; @endphp @enderror
                                </div>
                            </td>
                            </tr>
                            <tr class="hover:bg-gray-200 hover:shadow-md border-b-[1px] border-[#e4dfdf]">
                                <td class="px-4 py-4 whitespace-no-wrap">
                                    <label class="inline-flex items-center">
                                        <input type="checkbox" value="{{$i->izdavanje_id}}" name="vrati[]" class="form-checkbox">
                                    </label>
                                </td>
                                <td class="flex flex-row items-center px-4 py-4">
                                    ...
                                    <a href="ucenikProfile.php">
                                       ...
                                       {{$i->izdato_za($i->izdavanje_id)[0]->ImePrezime}}
                                        </span>
                                    </a>
                                </td>
                                <td class="px-4 py-4 text-sm leading-5 whitespace-no-wrap">
                                {{date('d.m.Y.',strtotime($i->datumizdavanja))}}</td>
                                <td class="px-4 py-4 text-sm leading-5 whitespace-no-wrap">
                                @if($i->zadrzavanje($i->izdavanje_id)['check']==true)
                               ...
                               {{$i->zadrzavanje($i->izdavanje_id)['danan']}}
                               @else 
                               @if(substr($i->zadrzavanje($i->izdavanje_id)['mjeseci'],0,1)=="0")
                               {{$i->zadrzavanje($i->izdavanje_id)['nedjelja']}}
                               {{$i->zadrzavanje($i->izdavanje_id)['danan']}}
                               @else 
                               {{substr($i->zadrzavanje($i->izdavanje_id)['danan'],2)}}
                               @endif
                               @endif
                               @else

                                {{$i->zadrzavanje($i->izdavanje_id)['dana']}}

                               @endif
                                </td>
                                <td class="px-4 py-4 text-sm leading-5 whitespace-no-wrap">
                               @if($i->prekoracenje($i->izdavanje_id)=='Nema prekoracenja')
                             <input class="disabled-btn" disabled type="hidden" name="prekoracenje{{$i->izdavanje_id}}" value="0"/>
                              Nema prekoracenja
                               @else
                               <input class="disabled-btn" disabled type="hidden" name="prekoracenje{{$i->izdavanje_id}}" value="1"/>
                               <span class="px-[6px] py-[2px] bg-red-200 text-red-800 rounded-[10px]">
                               {{$i->prekoracenje($i->izdavanje_id)}}      
                                </span>
                                @endif
                                </td>
                                <td class="px-4 py-4 text-sm leading-5 whitespace-no-wrap">  {{$i->izdato_od($i->izdavanje_id)[0]->ImePrezime}}</td>
                            </tr>
                           @endforeach
                        </tbody>
                    </table>
